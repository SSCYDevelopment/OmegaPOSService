...
AS
   ...
   (
       ...
       skoprc money,
       sknprc money,
       sktype char( 1 ) DEFAULT 'N',
       skyear smalldatetime,
       skptyp char( 2 ),
       skdmak decimal( 5, 2 ),
       skcomi decimal( 5, 2 ),
       skbbit int,
       skwght varchar( 2 ) DEFAULT '0'
   )
   ...
   */
   ...
   INSERT @tskun
          (skskun,skstyl,skskid,skstid,skcolr,sksize,skedes,skldes,skbran,skcate,skline,skseas,skcore,skoprc,sknprc,skyear,skwght)
   SELECT @pcSkun,smcode,@pcSkun,smstyl,@colorID,@sizeID,smedes,smldes,smbran,smcate,smline,smseas,smcore,smsprc,NULL,smyear,(case when smcore='W' then 1 else 0 END)
   FROM   mfstyl(nolock)
   ...
   */
   ...
   IF rtrim(@pcShop) <> ''
      BEGIN
         SELECT @ldToday = CONVERT(char(10), sdtxdt, 120) + ' ' + RIGHT(CONVERT(char(3), 100+datepart(hh, getdate())), 2) + ':' + RIGHT(CONVERT(char(3), 100+datepart(mi, getdate())), 2) + ':00'
         FROM   sydate(nolock)
   ...
   SELECT a.skskun SkuBarcde,a.skstyl StyleID,a.skcolr ColorID,a.sksize SizeID,a.skedes LocalDescription,a.skldes EnglishDescription,a.skbran Brand,a.skcate Category,a.skline Line,a.skoprc UnitPrice,a.sknprc Price,skwght IsWeight
   FROM   @tskun a 
go